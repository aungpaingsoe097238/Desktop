...
onMounted(async () => {
  ...
  currentImageId.value = database.getSkillData.length;
});
...
const handleSaveData = async (currentImageId) => {
  await database.setData("skill", `${currentImageId}`, {
    id: currentImageId,
    url: imageUrl.value,
  });
...
const handleDeleteData = async (imageId) => {
  await database.deleteData("skill", imageId);
  window.location.href = `${route.fullPath}`;
};
</script>
          ...
          <table class="w-full text-md text-left bg-white text-black">
            ...
            <tbody>
              <tr
                ...
                v-for="image in database.getSkillData"
                :key="image.id"
              >
